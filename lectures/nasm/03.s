...
case_flag_word:           equ                     0
case_flag_space:          equ                     1
...
_start:                                                ; Tell linker entry point
                        ...
                        mov                     r13, case_flag_space ; Set start value of case flag (prev)
...
; rax - the number of bytes that were read
                        ...
                        xor                     rcx, rcx ; Set start iterator value

.next_buffer_data:
                        mov                     r10b, BYTE [buffer + rcx] ; Current buffer byte-value
                        inc                     rcx ; ++ iterator

                        cmp                     r10b, 32 ; Check edge white space
                        je                      .white_space_case

                        sub                     r10b, 9 ; Check white space group
                        cmp                     r10b, 13 - 9 ; Check white space group
                        jng                     .white_space_case

                        add                     rbx, r13 ; ++ Answer counter

                        mov                     r13, case_flag_word ; Set case flag word
                        jmp                     .not_white_space_case
.white_space_case:
                        mov                     r13, case_flag_space ; Set case flag space

.not_white_space_case:
                        cmp                     rcx, rax ; Check end of buffer
                        je                      .read_next_buffer ; End of buffer (not input)

                        jmp                     .next_buffer_data
...
answer_stack_size       equ                     24 ; Default optimized stack size (8)
